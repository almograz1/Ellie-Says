...
export default function PhotoWordGame() {
  ...
  if (trialEnd) return (
    <Screen theme={theme}>
      ...
      <button onClick={() => router.push('/signin')} className="bg-purple-500 hover:bg-purple-600 text-white px-6 py-3 rounded-xl shadow">
        Sign Up
      </button>
    </Screen>
  );
  ...
  if (sessionEnd) return (
    <Screen theme={theme}>
      ...
      <button onClick={() => window.location.reload()} className="mt-4 bg-purple-500 hover:bg-purple-600 text-white px-6 py-3 rounded-xl shadow">
        Play Again
      </button>
    </Screen>
  );
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center p-6 text-center
      ${theme==='light'
        ? 'bg-gradient-to-br from-pink-200 via-purple-200 to-yellow-200'
        : 'bg-gradient-to-br from-indigo-900 via-pink-900 to-yellow-900'}`}>
      <div className={`backdrop-blur-md p-10 rounded-2xl shadow-2xl max-w-xl w-full
...
function Draggable({ char, id, theme }: Letter & { theme: string }) {
  ...
  return (
    <div
      ref={drag}
      className={`w-12 h-12 flex items-center justify-center rounded-lg border shadow text-xl font-bold
        ${isDragging ? 'opacity-30' : 'opacity-100'}
        ${theme==='light' ? 'bg-white border-purple-300 text-purple-800' : 'bg-gray-700 border-purple-500 text-purple-200'}`}
    >
      {char}
    </div>
  );
...
function Blank({
  ...
  index: number;
  letter: Letter | null;
  onDrop: (i: number, l: Letter) => void;
  theme: string;
  correct?: boolean;
}) {
  const [{ isOver }, drop] = useDrop(() => ({
    accept: ItemType.LETTER,
    drop: (it: Letter) => onDrop(index, it),
  ...
  return (
    <div
      ref={drop}
      className={`w-12 h-12 border-b-2 rounded-md flex items-center justify-center ${bg}
        ${theme==='light' ? 'border-purple-400' : 'border-purple-500'}`}
    >
      {letter && (
        <div ref={drag} className={isDragging ? 'opacity-30 cursor-move' : 'cursor-move'}>
          {letter.char}
        </div>
      )}
    </div>
...
function Pool({ letters, onDropBack, theme }: { letters: Letter[]; onDropBack: (l: Letter) => void; theme: string }) {
  ...
  return (
    <div ref={drop} className="flex flex-wrap justify-center gap-3">
      {letters.map(l => (
        <Draggable key={l.id} {...l} theme={theme} />
      ))}
    </div>
  );
}
